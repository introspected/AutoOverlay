...
- Скопировать AutoOverlay_netautoload.dll в папки с плагинами, один и тот же файл используется как для x86, так и x64.

### Описание
Плагин предназначен для оптимального наложения одного видеоклипа на другой.  
Сопоставление осуществляется путем тестирования различных координат наложения, разрешений, соотношений сторон и поворотов клипа с целью найти наилучшую комбинацию этих параметров. Функция сравнения областей двух кадров - среднеквадратическое отклонение (СКО, RMSE). Результат выполнения функции обозначается как diff (т.к. тестировались и другие функции).  
